...
function Logs() {
  ...
  const endpoint = `/system/logs?limit=200${level ? `&level=${encodeURIComponent(level)}` : ""}${service ? `&service=${encodeURIComponent(service)}` : ""}${query ? `&q=${encodeURIComponent(query)}` : ""}`;
  const result = useApi<{ logs?: RowData[] }>(endpoint, {});
  useEffect(() => {
    const timer = window.setInterval(result.reload, 3000);
    return () => window.clearInterval(timer);
  }, [endpoint]);
  const logs = result.data.logs || [];
  return <PageShell title="日志" action={<Space wrap><Select value={service || undefined} allowClear placeholder="组件" style={{ width: 120 }} onChange={(value) => setService(value || "")} options={[{ value: "bagualu", label: "八卦炉" }, { value: "mihomo", label: "Mihomo" }]} /><Select value={level || undefined} allowClear placeholder="级别" style={{ width: 120 }} onChange={(value) => setLevel(value || "")} options={[{ value: "error", label: "错误" }, { value: "warn", label: "警告" }, { value: "info", label: "信息" }, { value: "debug", label: "调试" }]} /><Input.Search value={draftQuery} allowClear placeholder="搜索任务、节点或错误" style={{ width: 240 }} onChange={(event) => setDraftQuery(event.target.value)} onSearch={setQuery} /><Button onClick={result.reload}>刷新</Button></Space>}>
    {result.error && <Alert type="error" showIcon message="日志加载失败" description={result.error} />}
    <Card><Table rowKey={(row) => String(row.id || row.job_id || row.time)} loading={result.loading} dataSource={logs} expandable={{ expandedRowRender: (row) => <AntDescriptions size="small" column={2} items={[{ key: "detail", label: "完整日志详情", span: 2, children: row.detail ? <Typography.Paragraph copyable={{ text: String(row.detail) }} style={{ margin: 0, whiteSpace: "pre-wrap" }}>{String(row.detail)}</Typography.Paragraph> : "—" }, { key: "service", label: "组件", children: String(row.service || "—") }, { key: "stream", label: "输出流", children: String(row.stream || "—") }, { key: "code", label: "错误码", children: String(row.error_code || "—") }, { key: "stage", label: "失败阶段", children: String(row.failure_stage || "—") }, { key: "status", label: "任务状态", children: String(row.status || "—") }, { key: "progress", label: "进度", children: `${String(row.progress || 0)}%` }, { key: "node", label: "节点 ID", children: <span className="mono">{String(row.node_id || "—")}</span> }, { key: "job", label: "完整任务 ID", children: <span className="mono">{String(row.job_id || "—")}</span> }, { key: "created", label: "创建时间", children: String(row.created_at || "—") }, { key: "finished", label: "结束时间", children: String(row.finished_at || "—") }]} /> }} columns={[{ title: "时间", dataIndex: "time", width: 180, render: formatMeasurementTime }, { title: "级别", dataIndex: "level", width: 80, render: (v) => <StateTag value={String(v || "")} /> }, { title: "服务", dataIndex: "service", width: 90 }, { title: "输出流", dataIndex: "stream", width: 90, render: (v) => String(v || "—") }, { title: "任务类型", dataIndex: "kind", width: 150 }, { title: "状态", dataIndex: "status", width: 100, render: (v) => <StateTag value={String(v || "")} /> }, { title: "错误码", dataIndex: "error_code", width: 180, render: (v) => String(v || "—") }, { title: "失败阶段", dataIndex: "failure_stage", width: 120, render: (v) => String(v || "—") }, { title: "消息", dataIndex: "message", render: (v) => <Typography.Text>{String(v || "—")}</Typography.Text> }, { title: "任务", dataIndex: "job_id", width: 100, render: (v) => <span className="mono">{String(v || "—").slice(0, 8)}</span> }]} locale={{ emptyText: <EmptyData title="暂无日志" hint="任务执行后会在这里显示状态记录。" /> }} pagination={{ pageSize: 15 }} /></Card>
  </PageShell>;
}
